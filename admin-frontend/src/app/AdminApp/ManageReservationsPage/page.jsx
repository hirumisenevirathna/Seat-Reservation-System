...
const Seat = ({ number, isAvailable, isReserved, isDisabled, fullName, onClick }) => {
  ...
  return (
    <div
      className={`w-10 sm:w-12 md:w-14 h-10 sm:h-12 md:h-14 rounded-xl flex flex-col items-center justify-center m-1 sm:m-1.5 md:m-2 shadow-lg transition-all duration-300 transform hover:scale-105 ${seatClass} cursor-pointer relative group`}
      title={statusText}
      onClick={onClick}
    >
      <span className="text-sm sm:text-base md:text-lg font-semibold">{number}</span>
      {isReserved && fullName && !isDisabled && (
        <span className="text-xs truncate w-8 sm:w-10 md:w-12">{fullName}</span>
      )}
...
export default function ManageReservationsPage() {
  ...
  const [action, setAction] = useState("add"); // 'add', 'edit', 'remove'
  ...
  const handleSeatClick = async (seatNumber) => {
    ...
    if (reservedSeats.includes(seatNumber)) {
      try {
        const res = await axios.get(`http://localhost:5000/api/seats/reservation?seatNumber=${seatNumber}&date=${date}`);
        setSelectedEmail(res.data.email || "");
        setSelectedReservationId(res.data._id);
      } catch (err) {
  ...
  const handleActionSubmit = async (e) => {
    ...
    if (["add", "edit"].includes(action) && !selectedEmail) {
      alert("Please enter an email for adding or editing a reservation");
      return;
    }
    ...
    try {
      if (action === "add") {
        ...
        const res = await axios.put(
          `http://localhost:5000/api/seats/reserve/${selectedReservationId || "new"}`,
          {
            seatNumber: selectedSeat,
            date,
            email: selectedEmail,
        ...
        alert(res.data.message || "Seat reservation updated successfully");
      } else if (action === "remove") {
        if (!selectedReservationId) {
          alert("No reservation found for this seat to remove.");
          return;
        }
        const confirmed = window.confirm(`Are you sure you want to remove the reservation for seat ${selectedSeat}?`);
        if (!confirmed) return;
        const res = await axios.delete(`http://localhost:5000/api/seats/reserve/${selectedReservationId}`);
        alert(res.data.message || "Seat reservation removed successfully");
      }
      fetchSeatsForDate(date);
      setSelectedSeat("");
      setSelectedEmail("");
  ...
  const renderSeatRow = (start, end) => (
    <div key={`row-${start}`} className="flex flex-wrap justify-center mb-3 sm:mb-4">
      {[...Array(Math.min(10, end - start + 1))].map((_, i) => {
  ...
  const renderSeatGrid = () => {
    const rows = [];
    for (let start = 1; start <= totalSeats; start += 10) {
      rows.push(renderSeatRow(start, Math.min(start + 9, totalSeats)));
      if (start + 10 <= totalSeats) {
        rows.push(
          <div
            key={`divider-${start}`}
            className="my-2 sm:my-4 h-1 bg-gradient-to-r from-gray-300 to-gray-500 rounded-full w-full max-w-3xl sm:max-w-4xl mx-auto"
          />
        );
      }
    }
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-3xl sm:max-w-4xl md:max-w-5xl bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 border border-white/20">
        ...
        <div className="mb-6 sm:mb-10 bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-white/20">
          <h2 className="text-xl sm:text-2xl font-semibold mb-4 sm:mb-6 text-center text-white drop-shadow-md">Manage Reservations</h2>
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
            {[
              { action: "add", label: "Add Reservation", icon: <FaPlus /> },
              { action: "edit", label: "Edit Reservation", icon: <FaEdit /> },
              { action: "remove", label: "Remove Reservation", icon: <FaTrash /> },
            ].map(({ action: btnAction, label, icon }) => (
              <button
                key={btnAction}
                onClick={() => setAction(btnAction)}
                className={`py-2 sm:py-3 px-4 sm:px-6 rounded-lg shadow-md transition-all duration-300 flex items-center gap-2 text-sm sm:text-base ${
                  action === btnAction
          ...
          <form onSubmit={handleActionSubmit} className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 justify-center">
            <select
              value={selectedSeat}
            ...
            <input
              ...
              className="border border-gray-300 rounded-lg px-3 sm:px-4 py-2 sm:py-3 bg-white/80 text-gray-800 shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300 w-full sm:w-48 md:w-64"
        ...
        <div className="rounded-xl p-4 sm:p-6 bg-white/10 backdrop-blur-md border border-white/20 mb-6 sm:mb-10">
          {renderSeatGrid()}
        </div>
